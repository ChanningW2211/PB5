...
par(mfrow=c(3, 3))
for (i in 2:9) {
  boxplot(allBeachesData[,i], main= names(allBeachesData)[i])
}
...
par(mfrow=c(3, 3))
for (i in 2:9) {
  plot(allBeachesData[,i], type = 'l',  main= names(allBeachesData)[i]) 
}
...
for (i in 2:9) {
    d =density(allBeachesData[,i], na.rm = TRUE)
    plot(d, type="n", main= names(allBeachesData)[i]) 
...
for (i in 2:9) {
    hist(allBeachesData[,i],  main= names(allBeachesData)[i], col = 'blue')
}
...
par(mfrow=c(1, 2))
WindData = allBeachesData[c(1,7,8)]
colnames(WindData) = c('date','wd','ws')
...
pairs20x(allBeachesData[2:6])

# scatter plot of Entero & Wind & solar
pairs20x(allBeachesData[c(2,7:8)])

```
######   Add a class column to the data
```{r}
allBeachesData$class = ifelse(allBeachesData$Entero < 280,0,1)
...
allBeachesData= allBeachesData[order(allBeachesData$DATE), ]
...
allBeachesDataClass = allBeachesData[,-c(1,2,6,10)]
...
trainDataLength = round(nrow(allBeachesDataClass)*(1-testProp),0)

train = allBeachesDataClass[1:trainDataLength,]
test =  allBeachesDataClass[(trainDataLength+1):nrow(allBeachesDataClass),]
...
filenametrain = "C:/Users/j/Documents/Uni/ComSci 760/Weather Project/Data/AllSitesOversampledTrainingData.csv"
...
for (i in 1:7) {
    hist(osTrain[,i], main= names(osTrain)[i], col = 'blue')
}
...
for (i in 1:7) {
    ...
    plot(d, type="n", main= names(osTrain)[i])
    polygon(d, col="red", border="gray")
}
```
...
Build model on non oversampled data
