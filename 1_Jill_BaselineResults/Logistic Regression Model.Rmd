...
cleanData = EntDataFull[,c(3:5,8,9,11,16)]
...
trainOS = mlr::smote(classifyTask, 12)[["env"]][["data"]]
...
gamFit = gam(Class ~ s(Rain24, 2) + s(Rain48, 2) + s(Rain72, 2) +s(Wspeed) + s(Solarhours,1) + BeachType + on_offshore, 
              data=trainOS, family=binomial(), select=TRUE) 
...
resids=gamFit$residuals
hist(resids)
qqnorm(resids)
qqline(resids)
main = 'Linear Model Normal Q-Q plot'
```

```{r}
# get y predicted (yhat) and plot residuals against yhat
ypredicted=gamFit$fitted.values
plot(ypredicted,resids)
abline(lm(resids~ypredicted), col="Red")
```

```{r}
# plot predicted agains actual
ypredicted=gamFit$fitted.values
plot(ypredicted,resids)
abline(lm(resids~ypredicted), col="Red")
```
...
####  Fit just the significant variables or ones chosen by Linda feature select

####  Drop Rain72 and Solarhours
...
gamFit2 = gam(Class ~ s(Rain24, 2) + s(Rain48, 2) +s(Wspeed, 1)  + BeachType + on_offshore, 
              data=trainOS, family=binomial(), select=TRUE) 


summary(gamFit2)
plot(gamFit2,  residuals = TRUE)
...
# get prediction
yhat_gamFit2  = as.numeric(predict(gamFit2, newdata=test)>0.5)

# show the confusion table
table(test$Class, yhat_gamFit2) 

# classification accuracy
(PE_yhat_gamFit2 = mean(test$Class == yhat_gamFit2))

# show the confusion table
(confusionMatrix = table(test$Class, yhat_gamFit2))
